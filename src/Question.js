...
export default function Question(props) {
    ...
    const buttonCorectStyles = {
        backgroundColor : props.isCorrect ? "#94D7A2" : "#F8BCBC"
    }
    
    return (
        <div className="question-container">
            <h2 className="question">{props.question}</h2>
            <button name={props.answers[0].answer} id={props.answers.indexOf(props.answers[0])} className="choice" onClick={(e) => props.handleAnswer(e)} value={props.questionIndex} style={props.isCorrect ? buttonCorectStyles : buttonOneStyles }>{props.answers[0].answer}</button>
            <button name={props.answers[1].wrong} id={props.answers.indexOf(props.answers[1])}className="choice" onClick={(e) => props.handleAnswer(e)} value={props.questionIndex} style={buttonTwoStyles}>{props.answers[1].wrong}</button>
